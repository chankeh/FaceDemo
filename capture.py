...
def catch_face(frame, tag):
    # 告诉OpenCV使用人脸识别分类器
    classfier = cv2.CascadeClassifier('/Library/Frameworks/Python.framework/Versions/3.8/lib/python3.8/site-packages/cv2/data/haarcascade_frontalface_alt2.xml')
...
def save_face(image, tag, num):
  # DATA_TRAIN为抓取的人脸存放目录，如果目录不存在则创建
    os.mkdir(os.path.join(config.DATA_TRAIN, str(tag)))
